...
async fn scan_port(port: u16, target_ip: String, tmp_file_name: String) {
...
pub async  fn scan_host(target_ip: String, ports: Vec<u16>, tmp_file_name: String) -> Vec<u16> {
    let port_box = futures::stream::iter(ports);
    //let mut open_ports: Vec<u16> = Vec::new();
    
    port_box.for_each_concurrent(10000, |port | {
        cli::print_scan_status(port.clone());
        scan_port(port, target_ip.clone(), tmp_file_name.clone())
        }).await;
    //for port in ports {
    /*
